...
### Flip/Rotation Augmentation Hurts Cross-Region SAR Generalization
Standard image augmentation (random horizontal/vertical flips) is common practice in computer vision and is used in the original [Sen1Floods11 paper](https://openaccess.thecvf.com/content_CVPRW_2020/html/w11/Bonafilia_Sen1Floods11_A_Georeferenced_Dataset_to_Train_and_Test_Deep_Learning_CVPRW_2020_paper.html) (Bonafilia et al., 2020) across all their compared models. However, SAR imagery has physically meaningful directional asymmetries that flips destroy — this matches an effect I first noticed qualitatively on TerraSAR-X data (flipped mountain/cliff terrain "looks wrong").
The likely mechanism is SAR's side-looking imaging geometry: because the sensor illuminates terrain obliquely from one side rather than looking straight down, slopes facing the sensor are compressed (foreshortening) or fold over on themselves (layover), while slopes facing away fall into radar shadow. This produces a consistent, direction-dependent brightness pattern tied to the sensor's look direction relative to terrain slope, not an arbitrary artifact. Flipping/rotating a tile scrambles that relationship, effectively teaching the model a physically inconsistent version of the signal it relies on to distinguish real terrain shadow from water (which is also low-backscatter/dark in SAR).
Testing this directly on Sen1Floods11 (resnet34 U-Net, identical architecture/data/threshold, only toggling flip augmentation on/off): on the Bolivia holdout region (never seen in training), disabling flip augmentation nearly doubled IoU (0.359 → 0.604 at threshold=0.5). The effect was much smaller on the in-distribution validation split, suggesting flips specifically harm cross-region transfer rather than in-distribution performance.
Since the Sen1Floods11 paper's headline generalization claim rests on that same Bolivia holdout, and flip augmentation was applied identically to all their compared models without ablation, their reported Bolivia numbers may be a suppressed version of what's achievable without it — a case where a common augmentation default doesn't transfer cleanly from natural images to physically-asymmetric radar imagery.
