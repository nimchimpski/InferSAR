...
class ProjectPaths:
    ...
    def validate_paths(self, job_type: str, require_checkpoint: bool = False):
        ...
        if require_checkpoint:
            required_paths.append((ckpt_dir, "Checkpoint input directory"))
        
        for path, description in required_paths:
            if not path.exists():
                errors.append(f"{description} not found: {path}")
        
        # Check for checkpoint files only when required for this run mode.
        if require_checkpoint and not any(ckpt_dir.rglob("*.ckpt")):
