...
def check_single_input_filetype(folder,  title, fsuffix):
    # logger.debug(f"---Checking for {title} in {folder}")
    # logger.debug(f"---Suffix: {fsuffix}")
    # logger.debug(f"---Title: {title}")
    input_list = [i for i in folder.iterdir() if i.is_file() and i.suffix.lower() == fsuffix.lower() and title.lower() in i.name.lower()]

    if len(input_list) == 0:
        logger.debug(f"---No file with '{title}' found in {folder}")
        return None
    elif len(input_list) > 1:
        logger.debug(f"---Multiple images found in {folder}. Using the first one. Delete the rest!")
        return None
    return input_list[0]

def path_not_exists(input_path):
    if not  input_path.exists():
        logger.debug(f"---no {input_path.name} found in {input_path}")
        return True
...
def rescale_image_minmax(image, min, max, output_path):
    ...
    logger.debug(f"Normalized image saved to {output_path}")
...
def compute_dataset_minmax(dataset, band_to_read=1):
    ...
    """
    logger.debug('+++in compute_dataset_minmax')
    ...
    for event in dataset.iterdir(): # ITER EVENT
        ...
        if event.is_dir():
            ...
            logger.debug(f'---num images= {len(images)}')
            if len(images) != 1:
            ...
            try:
                ...
                logger.debug(f'---global_min={global_min}, global_max={global_max}')
...
def check_loc_max_and_rescale(image, glob_max, output_path, band_to_read=1):
    with rasterio.open(image)as src:
        ...
        if locmax < 0.8*glob_max:
            ...
            logger.debug(f"---Rescaled {image.name} from {locmax} to {glob_max}")      
            logger.debug(f'---min { int(data.min())}, max { int(data.max())}')
            # Write the normalized image
            with rasterio.open(
                output_path,
                'w',
...
def check_and_rescale(data, metadata, glob_max, threshold=0.8):
    ...
    logger.debug(f"---Local min: {loc_min}, Local max: {loc_max}")
    data, scale_factor = rescale_image_minmax(data, glob_max, loc_max)
    logger.debug(f"---Rescaled from {loc_max} to {glob_max}")
...
def dataset_type(da):
    if isinstance(da, xr.Dataset):
        logger.debug('---da is a dataset')
    elif isinstance(da, xr.DataArray):
        logger.debug('---da is a dataarray')
    else:
        logger.debug('---da is not a dataset or dataarray')
...
def print_dataarray_info(da):
    logger.debug('++++++++++++PRINT DATARAY INFO--------------') 
    for layer in da.coords["layer"].values:
        layer_data = da.sel(layer=layer)
        logger.debug(f"---Layer '{layer}': Min={layer_data.min().item()}, Max={layer_data.max().item()}")
        logger.debug(f"---num unique vals = {len(np.unique(layer_data.values))}")
        if len(np.unique(layer_data.values)) < 4:
            logger.debug(f"---unique vals = {np.unique(layer_data.values)}")
        logger.debug(f'---Layer crs={layer_data.rio.crs}')  
        logger.debug('---------------------') 
    logger.debug('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n') 
...
def check_dataarray_list(dataarrays, layer_names):
    for i, da in enumerate(dataarrays):
        if not hasattr(da, 'rio'):
            logger.debug(f"---Error: {layer_names[i]} lacks rioxarray accessors. Reinitializing...")
        else:
            logger.debug('---has attr')
        logger.debug('---type da= ',type(da))  
        logger.debug(f"---Layer {i} name: {layer_names[i]}")
        logger.debug(f"---Shape: {da.shape}, CRS: {da.rio.crs}, Resolution: {da.rio.resolution()}, Bounds: {da.rio.bounds()}")
        if da.rio.crs != dataarrays[0].rio.crs:
            logger.debug(f"---Mismatch in CRS for {layer_names[i]}")
        if da.rio.resolution() != dataarrays[0].rio.resolution():
            logger.debug(f"---Mismatch in Resolution for {layer_names[i]}")
        dataarrays[i] = da.astype('float32')
        # chack the datatype
        logger.debug(f"---Data Type: {da.dtype}")
...
def print_tiff_info_TSX( image):
    logger.debug(f'+++ logger.debug TIFF INFO ---{image.name}')
    if image:
        ...
        with rasterio.open(image) as src:
            ...
            for i in range(1, src.count + 1):
                ...
                logger.debug(f"---Band {name}: Min={min}, Max={max}")
                logger.debug(f"---num unique vals = {numvals}")
                logger.debug(f"---CRS: {src.crs}")
                logger.debug(f'---resolution= {src.res}')
        
def check_single_tile(tile):
    with rasterio.open(tile) as src:
        # logger.debug('---tile:', tile.name)
        ...
        for band in range(1, src.count + 1):
            ...
            if name in ['mask', 'extent']:
                ...
                if numvals >2:
                    logger.debug('---not 2 vals , ', numvals)
                    return
                  # CHECK VALS ARE 0 OR 1
                min, max = min_max_vals(band_data)
                if round(min) not in [0, 1] or round(max) not in [0, 1]:
                    logger.debug(f'---min={min}, max={min}')
                    pass
            else:
                # logger.debug(f'--num_band_vals={numvals}')
                # CHECK MIN MAX INSIDE 0 AND 1 - NORMALIZED
                min, max = min_max_vals(band_data)
                if min == max:
                    logger.debug(f'---uniform values in {name} band: {min}, {max}')
                if min < 0 or max > 1:
                    logger.debug(f'---out of range values in {name} band: {min}, {max}')
...
def rasterize_kml_rasterio(kml_path, output_path, pixel_size=0.0001, burn_value=1):
    ...
    logger.debug(f"+++++Rasterizing extent from {kml_path} to {output_path}")
    ...
    logger.debug(f"Rasterized extent saved to {output_path}")
...
def calc_ratio(tiles):
    ...
    for tile in tqdm(tiles.iterdir(), total=len(list(tiles.iterdir()))):
        if tile.suffix != ".tif":
            continue
        # logger.debug(f"---Processing {tile.name}")
        with rasterio.open(tile) as src:
            data = src.read(3)
            flooded_count += np.sum(data == 1)
            non_flooded_count += np.sum(data == 0)
            # logger.debug(f'---flooded_count: {flooded_count}')
    ...
    # logger.debug(f'---event: {event.name}')
    logger.debug(f"{tile.parent.name} Ratio: {class_ratio:.2f}")
    return class_ratio